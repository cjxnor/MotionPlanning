...
def extract_path(closed, ngoal, nstart):
    rx, ry, ryaw, direc = [], [], [], []    # yaw是航向，direc代表车辆前进/后退
    cost = 0.0
    node = ngoal
...
# u：steer，d：direction  计算下个node，同时计算其cost
def calc_next_node(n_curr, c_id, u, d, P):
    ...
    if d > 0:
        ...
        direction = -1
        cost += abs(step) * C.BACKWARD_COST     # 倒车代价

    if direction != n_curr.direction:  # switch back penalty  挡位切换代价
        cost += C.GEAR_COST

    cost += C.STEER_ANGLE_COST * abs(u)  # steer angle penalyty  方向盘转角代价
    cost += C.STEER_CHANGE_COST * abs(n_curr.steer - u)  # steer change penalty  方向盘转角变化代价
...
def update_node_with_analystic_expantion(n_curr, ngoal, P):
    ...
    # 这里的calc_rs_path_cost()相当于计算这个节点的heuristic代价
...
def analystic_expantion(node, ngoal, P):
    ...
    paths = rs.calc_all_paths(sx, sy, syaw, gx, gy, gyaw, maxc, step_size=C.MOVE_STEP) # MOVE_STEP = 0.4
...
def is_collision(x, y, yaw, P):
    for ix, iy, iyaw in zip(x, y, yaw):
        ...
        for i in ids:
            # xo, yo是世界坐标系下的点坐标，转换到车辆坐标系下，cx, cy是世界坐标系下的车辆坐标
            xo = P.ox[i] - cx
            yo = P.oy[i] - cy
